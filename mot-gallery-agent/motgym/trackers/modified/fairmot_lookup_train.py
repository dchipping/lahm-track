...
class AgentSTrack(BaseTrack):
    ...
    def prune_gallery(self):
        # Remove most similar feature
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        # new_track.curr_feat /= np.linalg.norm(new_track.curr_feat)
...
class TrainAgentJDETracker():
    ...
    def update(self, dets, id_feature, frame_id):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [AgentSTrack(AgentSTrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f, min_iou_score,
                            agent=None) for (tlbrs, f, min_iou_score) in zip(dets[:, :5], id_feature, min_iou_scores)]
        else:
            detections = []
